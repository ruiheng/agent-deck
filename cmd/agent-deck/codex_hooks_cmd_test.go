...
func TestHandleCodexNotify_PromptSubmitDoesNotPublishRunning(t *testing.T) {
	const instanceID = "inst-prompt-submit"
	setupCodexNotifyInvocation(t, instanceID, []string{"agent-deck", "codex-notify"}, nil)

	// A previous turn may have left a fresh completion status in the shared
	// file. A blocked prompt must replace that published state with generation
	// evidence only, so it cannot look like a new running turn or completion.
	writeCodexHookStatus(instanceID, "waiting", "session-prompt", "agent-turn-complete", "turn-old")

	os.Args = []string{"agent-deck", "codex-notify"}
	setCodexNotifyStdin(t, []byte(`{"hook_event_name":"UserPromptSubmit","session_id":"session-prompt","turn_id":"turn-new"}`))
	handleCodexNotify()

	hook := readCodexNotifyHook(t, instanceID)
	if hook.Status != "" {
		t.Fatalf("prompt submission published status %q, want generation-only status", hook.Status)
	}
	if hook.Event != "UserPromptSubmit" {
		t.Fatalf("event = %q, want UserPromptSubmit", hook.Event)
	}
	if hook.CodexStartedGeneration != "session-prompt:turn-new" {
		t.Fatalf("started generation = %q, want new turn", hook.CodexStartedGeneration)
	}
	if hook.CodexCompletedGeneration != "" {
		t.Fatalf("stale completion generation survived prompt submission: %q", hook.CodexCompletedGeneration)
	}
}
