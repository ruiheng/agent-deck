...
func hookPayloadJSON(t *testing.T, fields map[string]any) string {
	t.Helper()
...
func TestHookHandler_MissingProjectDir_DegradesNotFatal(t *testing.T) {
	...
	payload := hookPayloadJSON(t, map[string]any{
	...
	escapedMissingDir, _ := json.Marshal(missingDir)
	if !strings.Contains(body, missingDir) && !strings.Contains(body, string(escapedMissingDir[1:len(escapedMissingDir)-1])) {
