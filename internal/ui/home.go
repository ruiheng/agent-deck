...
func (h *Home) handleMainKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "E":
		...
		if selected := h.getSelectedSession(); selected != nil && selected.IsSandboxed() &&
			...
			h.isAttaching.Store(true)
			return h, tea.Exec(attachCmd{session: sessionbackend.NewBackend(termSession), opts: tmux.AttachOptions{DetachByte: h.detachByte()}}, func(err error) tea.Msg {
				h.isAttaching.Store(false)
				return statusUpdateMsg{}
			})
		}
...
func (h *Home) attachSession(inst *session.Instance) tea.Cmd {
	...
	res := &attachResult{}
	return tea.Exec(attachCmd{session: inst.GetSessionBackend(), opts: h.attachOptions(), result: res}, func(err error) tea.Msg {
