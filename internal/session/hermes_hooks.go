...
func (l *hermesConfigLock) Release() {
	if l.file != nil {
		// Best-effort: unlock errors are non-actionable; Close drops the fd
		// either way, which also releases the lock on supported platforms.
		_ = unlockHermesConfigFile(l.file)
		_ = l.file.Close()
	}
	if l.inProc != nil {
		l.inProc.Unlock()
...
func acquireHermesConfigLock(configPath string) (*hermesConfigLock, error) {
	...
	if err := lockHermesConfigFile(f); err != nil {
		_ = f.Close()
		m.Unlock()
