...
func TestBuildClaudeCommand_WorkerScratchOverrideEmitsInfoLog(t *testing.T) {
	...
	escapedScratch := strings.ReplaceAll(scratch, `\`, `\\`)
	if !strings.Contains(output, profile) && !strings.Contains(output, escapedProfile) {
		t.Errorf("override log must include the resolved (overridden) config_dir %q; got: %s", profile, output)
	}
	if !strings.Contains(output, scratch) && !strings.Contains(output, escapedScratch) {
